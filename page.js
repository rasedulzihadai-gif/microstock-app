...
import { buildAdobeStockCsv, buildShutterstockCsv, buildFreepikCsv, buildIstockGettyCsv, getAdobeFilenameWarnings, downloadCsv } from "../lib/csv";
...
export default function Home() {
  ...
  function exportCsv(platform) {
    ...
    if (platform === "adobe_stock") {
      ...
      const longNames = getAdobeFilenameWarnings(doneItems);
      if (longNames.length > 0) {
        alert(
          `Heads up: Adobe Stock caps filenames at 30 characters. ${longNames.length} file(s) exceed that and will be rejected unless you rename the actual files (not just the CSV row) before uploading:\n\n` +
          longNames.join("\n")
        );
      }
    }
